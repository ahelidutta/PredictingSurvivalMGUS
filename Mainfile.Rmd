...
df_mgus <- survival::mgus2 %>% 
  select(age,dxyr,hgb,creat,mspike,pstat)
...
mgus_lg_df <- subset(mgus_lg_df, select = -c(death, futime, ptime, id)) %>% 
  initial_split(., strata = pstat)
mgus_lg_df
lg.train <- training(mgus_lg_df)
lg.test <- testing(mgus_lg_df)

#lg.train$age_bucket <- as.factor(ifelse(lg.train$age<=40,1,
                                      #  ifelse(lg.train$age<=60,2,
                                          #     ifelse(lg.train$age<=80,3,4))))

#lg.test$age_bucket <- as.factor(ifelse(lg.test$age<=40,1,
                                        #ifelse(lg.test$age<=60,2,
                                         #      ifelse(lg.test$age<=80,3,4))))
...
#summary(model_glm)
#model_2 <- stepAIC(model_glm)
#summary(model_2)
#summary(model_2$fitted.values)
#hist(model_2$fitted.values,main = " Histogram ",xlab = "Probability of '1' pstat", col = 'light green')
```

```{r}
library(pROC)
library(runway)
model <- glm(pstat~ age + hgb+ mspike, data = lg.train, family = "binomial",control = list(maxit = 100 ))
